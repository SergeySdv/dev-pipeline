# Frontend → Existing API Mapping

This mapping documents how the web console (`frontend/`) maps onto the existing FastAPI implementation in this repo.

## Primary API to target
The web console calls a REST API that is already implemented end-to-end by:
- `tasksgodzilla/api/app.py` (FastAPI “TasksGodzilla Orchestrator API”)

The `devgodzilla/api/*` service in this repo is a separate API with different endpoints/shapes (historically paired with a Windmill-native console app that has since been removed from this repo).

For the console (`frontend/`) the shortest path is to wire it to the **TasksGodzilla API** and then add **Windmill-backed proxies** for run logs + artifact content where needed.
