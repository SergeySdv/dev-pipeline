...
def setup_logging(level: Optional[str] = None, json_output: bool = False) -> logging.Logger:
    resolved_level = level or os.environ.get("DEKSDENFLOW_LOG_LEVEL", "INFO")
...
def init_cli_logging(level: Optional[str] = None, json_output: bool = False) -> logging.Logger:
    ...
    """
    return setup_logging(level or os.environ.get("DEKSDENFLOW_LOG_LEVEL", "INFO"), json_output=json_output)


def json_logging_from_env() -> bool:
    return os.environ.get("DEKSDENFLOW_LOG_JSON", "").lower() in ("1", "true", "yes")
