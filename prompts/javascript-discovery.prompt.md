...
- `specs/discovery/_runtime/DISCOVERY.md`: Node.js version, frameworks (Express/Koa/Next.js/React/Vue), package management (npm/yarn/pnpm), bundlers, dependencies, env vars/secrets, test fixtures, third-party services.
- `specs/discovery/_runtime/ARCHITECTURE.md`: high-level system/flow overview (components, modules, data/control flow, runtime/infrastructure, database/storage, auth/security, deployment notes).
- `specs/discovery/_runtime/API_REFERENCE.md`: callable surfaces in this repo (HTTP endpoints, CLI commands, functions/exports); include paths/verbs/flags, sample requests/responses or usage examples, auth/permissions, expected inputs/outputs.
...
5) Summaries: list remaining TODOs, missing JavaScript tools/deps, and assumptions in `specs/discovery/_runtime/CI_NOTES.md`.