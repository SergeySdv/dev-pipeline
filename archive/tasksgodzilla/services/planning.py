...
def _coerce_json_text(text: str) -> str:
    """
    Engines sometimes return JSON wrapped in Markdown code fences (```json ... ```).
    Coerce common wrappers into a raw JSON string suitable for json.loads().
    """
    ...
    def plan_protocol(self, protocol_run_id: int, job_id: Optional[str] = None) -> None:
        ...
        try:
            ...
            for attempt in range(2):
                ...
                candidate = json.loads(_coerce_json_text(planning_json))
            ...
            if isinstance(exc, ConfigError):
                self._stub_plan(
                    ...
                    workspace_hint,
                    protocol_hint,
                    f"{engine.metadata.id} unavailable",
                    job_id=job_id,
                )
                return
            self.db.update_protocol_status(run.id, ProtocolStatus.BLOCKED)
