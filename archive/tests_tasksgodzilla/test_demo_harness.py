...
def test_demo_harness_covers_discovery_to_validation(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setenv("TASKSGODZILLA_DEFAULT_ENGINE_ID", "opencode")
    ...
    # Treat engines as unavailable so execution paths use the stubbed flow.
    orig_which = shutil.which
    monkeypatch.setattr(
        codex_worker.shutil,
        "which",
        lambda name, _orig=orig_which: None if name in ("codex", "opencode") else _orig(name),
    )
    import tasksgodzilla.engines_opencode as opencode_mod
    import tasksgodzilla.services.onboarding as onboarding_mod

    monkeypatch.setattr(
        opencode_mod.shutil,
        "which",
        lambda name, _orig=orig_which: None if name == "opencode" else _orig(name),
    )
    monkeypatch.setattr(
