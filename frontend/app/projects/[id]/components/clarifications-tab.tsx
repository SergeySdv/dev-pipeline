...
import { useProjectClarifications, useAnswerClarification } from "@/lib/api"
...
export function ClarificationsTab({ projectId }: ClarificationsTabProps) {
  ...
  const { data: clarifications, isLoading } = useProjectClarifications(projectId, statusFilter || undefined)
  ...
  const blockingCount = clarifications?.filter((c) => c.status === "open" && c.blocking).length || 0
      ...
      {!clarifications || clarifications.length === 0 ? (
        ...
        <div className="space-y-4">
          {clarifications.map((clarification) => (
            <ClarificationCard key={clarification.id} clarification={clarification} projectId={projectId} />
          ))}
        </div>
      )}
    </div>
...
function ClarificationCard({ clarification, projectId }: { clarification: Clarification; projectId: number }) {
    ...
    <Card className={clarification.blocking && clarification.status === "open" ? "border-yellow-500/50" : ""}>
      ...
      <CardContent className="space-y-4">
        ...
        {clarification.recommended && (
          <p className="text-sm text-muted-foreground">
            Recommended: <span className="font-medium">{typeof clarification.recommended === "string" ? clarification.recommended : JSON.stringify(clarification.recommended)}</span>
          </p>
        )}
        ...
        {clarification.status === "open" ? (
          ...
          <div className="rounded-lg bg-muted p-3">
            <p className="text-sm">
              <span className="font-medium">Answer:</span> {typeof clarification.answer === "string" ? clarification.answer : JSON.stringify(clarification.answer)}
            </p>
