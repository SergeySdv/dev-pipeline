...
export default function ProjectProtocolsPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold">{project.name} - Protocols</h1>
          <p className="text-muted-foreground text-sm">{project.git_url}</p>
        </div>
      </div>

      <ProtocolsTab projectId={projectId} />
    </div>
  );
}
