...
import { useAnswerClarificationById, useClarifications, useProjects } from "@/lib/api";
      ...
      <div className="flex h-full flex-col gap-6 p-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Answer open questions across projects and protocols
          </p>
        </div>
        <EmptyState
          icon={MessageCircle}
    ...
    <div className="flex h-full flex-col gap-6 p-6">
      ...
      <div className="grid gap-4">
        {clarifications.map((clarification) => {
          const projectName = clarification.project_id
            ? projectMap.get(clarification.project_id)
            : null;
          ...
          return (
            <Card
              ...
              className={clarification.blocking && isOpen ? "border-amber-500/40" : ""}
            >
                ...
                <div className="flex items-start justify-between gap-4">
                    ...
                    <CardDescription>
                      {projectName ? (
                        <span>
                          ...
                          <Link
                            className="underline"
                            href={`/projects/${clarification.project_id}`}
                          >
                            {projectName}
                          </Link>
                        </span>
                      ...
                      {clarification.protocol_run_id && (
                        <>
                          ...
                          <Link
                            className="underline"
                            href={`/protocols/${clarification.protocol_run_id}`}
                          >
                            {clarification.protocol_run_id}
                          </Link>
                        </>
                  ...
                  <div className="flex items-center gap-2">
                    {clarification.applies_to && (
                      <Badge variant="secondary">{clarification.applies_to}</Badge>
                    )}
                ...
                {isOpen ? (
                  <div className="flex flex-wrap gap-2">
                    ...
                    <Button
                      ...
                      disabled={
                        answerClarification.isPending || !(drafts[clarification.id] || "").trim()
                      }
                    >
