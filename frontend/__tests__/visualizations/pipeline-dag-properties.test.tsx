...
const stepRunArbitrary = (
  id: number,
  stepIndex: number,
  dependsOn: number[] = []
): fc.Arbitrary<StepRun> =>
  fc.record({
    id: fc.constant(id),
...
const stepsListArbitrary = fc.integer({ min: 1, max: 10 }).chain((count) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    stepArbitraries.push(depsArbitrary.chain((deps) => stepRunArbitrary(i + 1, i, deps)));
  }
...
describe("Pipeline DAG Property Tests", () => {
      ...
      fc.assert(
        fc.property(
          fc
            ...
            .filter((s) => !["running", "completed", "failed", "pending"].includes(s)),
          (unknownStatus) => {
  ...
  describe("Property 6: DAG duration calculation", () => {
    ...
    it("should handle zero duration", () => {
      ...
      fc.assert(
        fc.property(fc.integer({ min: minTime, max: maxTime }), (timeMs) => {
