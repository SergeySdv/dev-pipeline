      ...
      fc.assert(
        fc.property(testRowsArbitrary, searchQueryArbitrary, (rows, query) => {
          ...
          const expectedIds = rows
            .filter(r => !normalizedQuery || JSON.stringify(r).toLowerCase().includes(normalizedQuery))
            .map(r => r.id)
    ...
    it('should be case-insensitive', () => {
      fc.assert(
        fc.property(
          ...
          (rows, query) => {
            const lowerFiltered = filterTableData(rows, query.toLowerCase(), [], standardAccessorKeyMap)
            const upperFiltered = filterTableData(rows, query.toUpperCase(), [], standardAccessorKeyMap)
      ...
      fc.assert(
        fc.property(
          ...
          (rows, globalSearch, columnFilter) => {
            ...
            const filtered = filterTableData(rows, globalSearch, columnFilters, standardAccessorKeyMap)
