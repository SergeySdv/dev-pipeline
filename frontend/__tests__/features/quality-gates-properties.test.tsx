...
const articleArbitrary = fc
  .string({ minLength: 1, maxLength: 20 })
  .filter((s) => s.trim().length > 0);

// Arbitrary generator for gate names
const gateNameArbitrary = fc
...
describe("Quality Gates Drilldown Property Tests", () => {
  ...
  describe("Property 11: Quality gates listing completeness", () => {
    ...
    it("should return highest severity from multiple findings", () => {
      ...
      fc.assert(
        fc.property(fc.array(findingArbitrary, { minLength: 1, maxLength: 10 }), (findings) => {
          ...
          const expectedMaxIndex = Math.min(
            ...findingSeverities.map((s) => severityOrder.indexOf(s))
          );
