...
const agentIdArbitrary = fc.string({ minLength: 1, maxLength: 50 })
  .filter(s => s.trim().length > 0)

// Arbitrary generator for agent names
const agentNameArbitrary = fc.string({ minLength: 1, maxLength: 100 })
  .filter(s => s.trim().length > 0)
...
const agentStatusArbitrary = fc.constantFrom('available', 'busy', 'unavailable') as fc.Arbitrary<'available' | 'busy' | 'unavailable'>
...
const agentHealthArbitrary = (agentId: string): fc.Arbitrary<AgentHealth> => fc.record({
...
const agentMetricsArbitrary = (agentId: string): fc.Arbitrary<AgentMetrics> => fc.record({
  agent_id: fc.constant(agentId),
  active_steps: fc.nat({ max: 100 }),
  completed_steps: fc.nat({ max: 1000 }),
  failed_steps: fc.nat({ max: 100 }),
  total_steps: fc.nat({ max: 1200 }),
  last_activity_at: fc.option(fc.date().map(d => d.toISOString()), { nil: undefined }),
})
