...
import type { AgileTaskCreate, TaskType, TaskPriority, TaskBoardStatus } from '@/lib/api/types'

// Arbitrary generator for task types
const taskTypeArbitrary: fc.Arbitrary<TaskType> = fc.constantFrom('story', 'bug', 'task', 'spike', 'epic')

// Arbitrary generator for task priorities
const taskPriorityArbitrary: fc.Arbitrary<TaskPriority> = fc.constantFrom('critical', 'high', 'medium', 'low')

// Arbitrary generator for board statuses
const boardStatusArbitrary: fc.Arbitrary<TaskBoardStatus> = fc.constantFrom('backlog', 'todo', 'in_progress', 'review', 'testing', 'done')

// Arbitrary generator for non-empty strings (valid titles)
const validTitleArbitrary = fc.string({ minLength: 1, maxLength: 200 })
...
describe('Task Form Validation Property Tests', () => {
  ...
  describe('Property 16: Task form validation', () => {
    ...
    it('should return consistent validation results for the same input', () => {
      fc.assert(
        fc.property(
          fc.oneof(validTaskFormArbitrary, invalidTaskFormArbitrary),
          (formData) => {
