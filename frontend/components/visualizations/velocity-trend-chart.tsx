...
function VelocityTooltip({ active, payload }: VelocityTooltipProps) {
  ...
  return (
    <div className="bg-background border-border rounded-md border p-3 text-sm shadow-md">
      <p className="mb-2 font-medium">{data.sprintName}</p>
...
export function VelocityChart({
  ...
  if (!chartData || chartData.length === 0) {
    return (
      <div className={cn("text-muted-foreground py-6 text-sm", className)}>
    ...
    <div className={cn(className)} style={{ height }}>
      <ResponsiveContainer width="100%" height="100%">
        <ComposedChart data={chartData} margin={{ top: 8, right: 16, bottom: 8, left: 0 }}>
          ...
          <XAxis dataKey="sprintName" tick={{ fontSize: 11 }} className="text-muted-foreground" />
          <YAxis tick={{ fontSize: 11 }} width={36} className="text-muted-foreground" />
