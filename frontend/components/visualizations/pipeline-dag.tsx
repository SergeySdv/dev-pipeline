...
import { coordCenter,decrossOpt, graphStratify, sugiyama } from "d3-dag";
...
export function PipelineDag({ steps, onStepClick, className, selectedStepId }: PipelineDagProps) {
  ...
  useEffect(() => {
    ...
    nodeElements
      .filter((d) => d.step.id === selectedStepId)
    ...
    nodeElements
      .filter((d) => !!d.step.parallel_group)
      .append("text")
      .attr("x", nodeWidth - 10)
      .attr("y", 20)
  ...
  if (steps.length === 0) {
    return (
      <div
        className={cn("bg-muted/20 text-muted-foreground rounded-lg border p-6 text-sm", className)}
      >
        No steps to display
      </div>
    );
  ...
  return (
    ...
    >
      <div className="bg-muted/30 text-muted-foreground flex items-center justify-between border-b p-3 text-xs">
        <span>DAG View • Scroll to zoom, drag to pan • Click a node to view details</span>
        <div className="flex items-center gap-4">
          <span className="flex items-center gap-1">
            <span className="h-3 w-3 rounded-full bg-blue-500"></span> Running
          </span>
          <span className="flex items-center gap-1">
            <span className="h-3 w-3 rounded-full bg-green-500"></span> Completed
          </span>
          <span className="flex items-center gap-1">
            <span className="h-3 w-3 rounded-full bg-red-500"></span> Failed
          </span>
          <span className="flex items-center gap-1">
            <span className="h-3 w-3 rounded-full bg-gray-400"></span> Pending
          </span>
        </div>
      </div>
      <svg
