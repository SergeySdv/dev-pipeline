...
export function validateQueueStatsBreakdown(breakdown: QueueStatusBreakdown): QueueStatsValidation {
  ...
  const allCountsNonNegative =
    breakdown.queued >= 0 && breakdown.started >= 0 && breakdown.failed >= 0;
...
export function QueueStatsPanel() {
  ...
  const { data: jobs = [], isLoading: jobsLoading } = useQueueJobs(
  ...
  const jobColumns: ColumnDef<QueueJob>[] = [
    {
      accessorKey: "job_id",
      header: "Job ID",
      cell: ({ row }) => <span className="font-mono text-xs">{row.original.job_id}</span>,
    },
    {
    ...
    },
    {
      ...
      header: "Enqueued",
      cell: ({ row }) => (
        <span className="text-muted-foreground text-xs">{row.original.enqueued_at}</span>
      ),
    },
  ];
