...
export function validateQualityGateListItemCompleteness(itemData: QualityGateListItemData): {
  ...
  const hasSeverityIndicator = itemData.findingsCount === 0 || itemData.hasSeverityIndicator || itemData.maxSeverity === "unknown"
                ...
                <div className="rounded-lg border">
                  <div className="flex items-center justify-between gap-3 p-3">
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 text-xs text-muted-foreground">Findings: {itemData.findingsCount}</div>
                    </div>
                  </div>
                    ...
                    {itemData.findingsCount === 0 ? (
                      ...
                      <div className="space-y-2">
                        {itemData.findings.map((finding, idx) => {
                          ...
                          return (
                            <div key={idx} className="rounded border bg-muted/40 p-3">
                              <div className="flex items-start gap-2">
                                {finding.severity && (
                                  <Badge
                                    variant={findingBadgeProps.variant}
                                    className={cn("text-[10px] shrink-0", findingBadgeProps.className)}
                                  >
